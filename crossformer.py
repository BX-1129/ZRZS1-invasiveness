    ...
    def __init__(
        ...
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                Attention(dim, attn_type='short', window_size=local_window_size, dim_head=dim_head,
                          dropout=attn_dropout),
                FeedForward(dim, dropout=ff_dropout),
                Attention(dim, attn_type='long', window_size=global_window_size, dim_head=dim_head,
                          dropout=attn_dropout),
                FeedForward(dim, dropout=ff_dropout)
            ]))
...
class CrossFormer(nn.Module):
    def __init__(
        ...
        super().__init__()
        logger.info(f'正在使用 CrossFormer，具体参数为：global_window_size ={global_window_size}, '
                    f'dim={dim}, '
        ...
        # final logits
        self.avgpool = Reduce('b c h w -> b c', 'mean')
