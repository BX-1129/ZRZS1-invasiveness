...
def cluster(ori_dirs, m_list, save_dir, mask_id: int=None, n_clusters=5, norm: bool=True):
    ...
    for ncluster in n_clusters:
        ...
        try:
            db_score = davies_bouldin_score(data, model.labels_)
        except Exception as e:
            print(f"警告: 无法计算 Davies-Bouldin index: {e}")
            db_score = float('nan')

        # Silhouette coefficient (越大越好，范围[-1, 1])
        try:
            # 数据量较大时采样计算以提升速度，最多取10000个样本
            if data.shape[0] > 10000:
                sample_idx = np.random.choice(data.shape[0], size=10000, replace=False)
                sil_score = silhouette_score(data[sample_idx], model.labels_[sample_idx], random_state=0)
            else:
                sil_score = silhouette_score(data, model.labels_)
        except Exception as e:
            print(f"警告: 无法计算 Silhouette coefficient: {e}")
            sil_score = float('nan')
        # --------------------
        ...
        table.add_row(["Calinski-Harabasz score", f"{ch_score:.4f}"])
        table.add_row(["Davies-Bouldin index", f"{db_score:.4f}"])
        table.add_row(["Silhouette coefficient", f"{sil_score:.4f}"])
        
        metric_scores.append((ncluster, ch_score, db_score, sil_score))
        print(f"聚类的类别分布：\n{table}")
        cluster_idx = 0
...
def main(image_dirs, mask_dir, output_dir, num_cluster: Union[(int, List[int])]=3, separate: bool=False, norm: bool=True):
    ...
    if separate:
        for m in m_list:
            try:
                ...
                scores = cluster(image_dirs, [m], sample_dir, n_clusters=num_cluster, norm=norm)
                metric_scores.extend(scores)
            except Exception as e:
                print(f"处理样本 {os.path.basename(m)} 时出错: {e}")
    else:
        try:
            scores = cluster(image_dirs, m_list, output_dir, n_clusters=num_cluster, norm=norm)
            metric_scores.extend(scores)
        except Exception as e:
            print(f"聚类过程中出错: {e}")
            raise
    ...
    if len(metric_scores) > 0:
        metric_df = pd.DataFrame(metric_scores, columns=["n_clusters", "ch_score", "db_score", "sil_score"])
        metric_summary = metric_df.groupby("n_clusters").agg("mean").reset_index()
        table = PrettyTable()
        table.field_names = ["n_clusters", "CH score (↑)", "DB index (↓)", "Silhouette (↑)"]
        for row in np.array(metric_summary):
            table.add_row([int(row[0]), f"{row[1]:.4f}", f"{row[2]:.4f}", f"{row[3]:.4f}"])

        print(f"--------------- 平均聚类评估指标 ---------------\n{table}")
        print("注: CH score 越大越好；DB index 越小越好；Silhouette 越大越好(范围[-1,1])")
...
if __name__ == "__main__":
    ...
    try:
        ...
        result = main(xpath, ypath, o_dir, num_cluster=(args.cluster), separate=(args.separate), norm=(not args.ori_value))
        print(result)
        input("转化完成，按任意键退出！")
    except Exception as e:
        print(f"程序执行出错: {e}")
